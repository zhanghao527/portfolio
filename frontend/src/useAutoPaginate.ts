...
function projectCardHTML(p: ProjectItem): string {
  return `<div class="project-list-item">
    <div class="project-list-body">
      ...
      <div class="project-list-desc">${p.desc}</div>
    </div>
  </div>`
}
...
export function useAutoPaginate(bookWidth: number, bookHeight: number, dataVersion: number = 0): PaginateResult & { ready: boolean } {
    ...
    // ── Section 1: Projects (grouped by category) ──
    {
      ...
      for (const g of groups) {
        ...
        el.innerHTML = `
          ...
          <div class="project-list">${g.items.map(p => projectCardHTML(p)).join('')}</div>
        `
  ...
  useEffect(() => {
    const key = `${bookWidth}x${bookHeight}x${dataVersion}`
    if (key === prevKey.current) return
    prevKey.current = key
    // Delay to ensure CSS is loaded
    requestAnimationFrame(() => paginate())
  }, [paginate, dataVersion])
          ...
          createElement('div', { className: 'project-group', key: g.category },
            ...
            createElement('div', { className: 'project-list' },
              ...g.items.map(p => {
                ...
                const docIcon = createElement('svg', { width: 14, height: 14, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '2', strokeLinecap: 'round', strokeLinejoin: 'round' },
                ...
                const linkIcon = createElement('svg', { width: 14, height: 14, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: '2', strokeLinecap: 'round', strokeLinejoin: 'round' },
                ...
                const iconsEl = createElement('div', { className: 'project-list-icons' },
                ...
                return createElement('div', { className: 'project-list-item', key: p.name },
                  createElement('div', { className: 'project-list-body' },
                    createElement('div', { className: 'project-list-name' }, p.name),
                    createElement('div', { className: 'project-list-desc' }, p.desc),
                  ),
                  iconsEl,
                )
              })
          ...
          ...groups.map(g =>
            createElement('div', { className: 'tech-group', key: g.category },
              ...
              createElement('div', { className: 'tech-list' },
                ...g.items.map(t =>
                  createElement('span', { className: 'tech-tag', key: t.name }, t.name)
                )
              ),
            )
          ),
